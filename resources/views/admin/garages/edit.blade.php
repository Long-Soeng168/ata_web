...
@section('content')
    ...
    @endif
    <div class="p-4">
        <x-form-header :value="__('Update Garage')" class="p-4" />

        <form class="w-full" action="{{ route('admin.garages.update', $garage->id) }}" method="POST"
            ...
            <div class="grid mt-4 md:grid-cols-2 md:gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="location" class="block w-full mt-1" type="text" name="location"
                        value="{{ old('location', $garage->location) }}" placeholder="location" />
                    <x-input-error :messages="$errors->get('location')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="like" :value="__('Like')" />
                    <x-text-input id="like" class="block w-full mt-1" type="number" name="like"
                        value="{{ old('like', $garage->like) }}" placeholder="like" />
                    <x-input-error :messages="$errors->get('like')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="unlike" :value="__('UnLike')" />
                    <x-text-input id="unlike" class="block w-full mt-1" type="number" name="unlike"
                        value="{{ old('unlike', $garage->unlike) }}" placeholder="unlike" />
                    <x-input-error :messages="$errors->get('unlike')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="rate" :value="__('Rate')" />
                    <x-text-input id="rate" class="block w-full mt-1" type="number" name="rate"
