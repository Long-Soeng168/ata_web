...
<div>
    ...
    <x-page-header :value="__('Courses')"/>
    <div class="flex flex-col px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
        <div class="w-full md:w-1/2">
            <form action="{{ url('admin/courses') }}" class="flex items-center gap-4" method="GET">
                @csrf
        ...
        <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
            ...
            <x-primary-button href="{{ route('admin.courses.create') }}">
                ...
                @forelse ($items as $item)
                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                        ...
                        <th scope="row" class="flex items-center px-4 py-2 font-medium text-gray-900 dark:text-white">
                            <img src="{{ asset('assets/images/courses/thumb/'.$item->image) }}" alt="iMac Front Image" class="object-cover h-10 mr-3 aspect-video">
                        </th>
                        <x-table-data value="{{ $item->title }}"/>
                        <td class = 'px-4 py-2 font-medium text-gray-900 tex-sm dark:text-white '>
                            {{ $item->description }}
                        </td>
                        <x-table-data value="{{ $item->price }}"/>
                        <x-table-data value="{{ $item->start }}"/>
                        <x-table-data value="{{ $item->end }}"/>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-start justify-center gap-3">
                                ...
                                identifier="{{ $item->id }}"
                                viewDetailUrl="{{ route('admin.courses.show', $item->id) }}"
                                tooltipText=" View item details"
                                />
                                <x-delete-confirm-button
                                identifier="{{ $item->id }}"
                                deleteUrl="{{ route('admin.courses.destroy', $item->id) }}"
