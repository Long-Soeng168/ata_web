...
@section('content')
    <div class="p-4">
        <x-form-header :value="__('Create Course')" class="p-4" />

        <form class="w-full" action="{{ route('admin.courses.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div>
                    ...
                    <x-text-input id="start" class="block w-full mt-1" type="date" name="start" :value="old('start')"
                         autofocus placeholder="Start Date" />
                    <x-input-error :messages="$errors->get('start')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="end" :value="__('End')" />
                    <x-text-input id="end" class="block w-full mt-1" type="date" name="end" :value="old('end')"
