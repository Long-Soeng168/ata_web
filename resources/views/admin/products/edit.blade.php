...
<div class="p-4">
    <x-form-header :value="__('Update Product')" class="p-4"/>

    <form class="w-full" action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 md:gap-6">
            ...
            <div>
                ...
                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ old('name', $product->name) }}" required autofocus placeholder="Name" />
        ...
        <div class="grid md:grid-cols-3 md:gap-6 mt-4">
            ...
            <div>
                ...
                <x-text-input id="code" class="block mt-1 w-full" type="text" name="code" value="{{ old('code', $product->code) }}" placeholder="Code" />
                <x-input-error :messages="$errors->get('code')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="price" :value="__('Price')" />
                <x-text-input id="price" class="block mt-1 w-full" type="number" name="price" value="{{ old('price', $product->price) }}" required placeholder="Price" />
                <x-input-error :messages="$errors->get('price')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="discount" :value="__('Discount % ')" />
                <x-text-input id="discount" class="block mt-1 w-full" type="number" name="discount_percent" value="{{ old('discount_percent', $product->discount_percent) }}" placeholder="Discount" />
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <x-select-option id="model_id" name="model_id">
                    ...
                    @foreach ($models as $model)
                        <option value="{{ $model->id }}" {{ $model->id == old('model_id', $product->model_id) ? 'selected' : '' }}>{{ $model->name }}</option>
                    @endforeach
                </x-select-option>
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <x-select-option id="type_id" name="type_id">
                    ...
                    @foreach ($types as $type)
                        <option value="{{ $type->id }}" {{ $type->id == old('type_id', $product->type_id) ? 'selected' : '' }}>{{ $type->name }}</option>
                    @endforeach
                </x-select-option>
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <x-select-option id="body_type_id" name="body_type_id">
                    ...
                    @foreach ($body_types as $body_type)
                        <option value="{{ $body_type->id }}" {{ $body_type->id == old('body_type_id', $product->body_type_id) ? 'selected' : '' }}>{{ $body_type->name }}</option>
                    @endforeach
                </x-select-option>
        ...
        <div class="mb-5">
            <div class="flex items-center space-4">
                <div class="max-w-40">
                    <img id="selected-image" src="{{ $product->image ? asset('storage/' . $product->image) : '#' }}" alt="Selected Image" class="{{ $product->image ? '' : 'hidden' }} max-w-full max-h-40 pr-4" />
                </div>
                <div class="flex-1">
