        ...
        <form class="w-full" {{-- action="{{ route('admin.products.store') }}"
            ...
            <div class="mt-5 mb-5">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1">
                        <x-input-label required for="images" :value="__('Upload Images (max: 2MB each)')" />
                        <input type="file" id="dropzone-file" name="images[]"
                            accept="image/png, image/jpeg, image/gif" wire:model="images" multiple
                            onchange="displaySelectedImages(event)"
                            class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none" />

                        <div class="flex gap-2 pt-2" wire:loading wire:target="images">
